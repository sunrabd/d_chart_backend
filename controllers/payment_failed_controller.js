...
const createPaymentFailed = async (req, res) => {
  try {
    ...
    const paymentFailed = await PaymentFailed.create({ amount, userId, reason });
...
const getAllPaymentFailed = async (req, res) => {
  try {
    const payments = await PaymentFailed.findAll({ include: [{ model: User, as: 'user' }] });
...
const updatePaymentFailed = async (req, res) => {
  try {
    ...
    await payment.update({ amount, reason });
